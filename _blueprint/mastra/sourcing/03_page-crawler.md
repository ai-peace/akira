# pageCrawlerのstepを構築してください

## 依頼内容

ありがとうございます。
次は、いただいたキーワード検索のURLをベースに、
buildQueryでのキーワードとOptionデータ取得を接続し、
ページを巡回してProductEntityに変換し、DBに保存する処理を実行してください。
...
- buildQueryからキーワードとoptionsを受け取る
- ベースのURLを元にページを構築する
- デバッグ用:一時ディレクトリにページを保管する
- ページ解析をして、検索結果が何個あるかを把握する
- そのページ件数を元に取得するページ数を把握し、url群を生成する
- 順次url群にアクセスし、productEntityに速やかに変換し、取得したものをdbに追加していく(デッドロックに注意しながら)
